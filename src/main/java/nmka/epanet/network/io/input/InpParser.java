...
package nmka.epanet.network.io.input;


import nmka.epanet.Constants;
import nmka.epanet.network.FieldsMap;
import nmka.epanet.network.FieldsMap.Type;
import nmka.epanet.network.Network;
import nmka.epanet.network.PropertiesMap;
import nmka.epanet.network.io.Keywords;
import nmka.epanet.network.structures.*;
import nmka.epanet.network.structures.Control.ControlType;
import nmka.epanet.network.structures.Link.LinkType;
import nmka.epanet.network.structures.Link.StatType;
import nmka.epanet.util.ENException;
import nmka.epanet.util.ENLevels;
import nmka.epanet.util.Utilities;
...
public class InpParser extends InputParser {
    ...
    private void parsePC(Network net, File f) throws ENException {
        ...
        try {
            buffReader = new BufferedReader(new InputStreamReader(new FileInputStream(f), Charset.forName("GBK")));
        } catch (IOException e) {
    ...
    protected void parseReservoir(Network net, String[] Tok, String comment) throws ENException {
        int n = Tok.length;
        Pattern p = null;
        Curve c = null;
        Double el,
        ...
        if (comment.length() > 0)
            reservoir.setComment(comment);

        if (net.getNode(Tok[0]) != null)
            throw new ENException(215);

        net.addReservoir(Tok[0], reservoir);
        ...
        reservoir.setReportFlag(false);
        reservoir.setElevation(el);
        reservoir.setC0(new double[]{0.0d});
        reservoir.setSource(null);
        reservoir.setKe(0.0);

        reservoir.setH0(initlevel);
        reservoir.setHmin(minlevel);
        reservoir.setHmax(maxlevel);
        reservoir.setArea(diam);
        reservoir.setPattern(p);
        reservoir.setKb(Constants.MISSING);

        area = Constants.PI * diam * diam / 4.0d;

        reservoir.setVmin(area * minlevel);
        if (minvol > 0.0)
            reservoir.setVmin(minvol);

        reservoir.setV0(reservoir.getVmin() + area * (initlevel - minlevel));
        reservoir.setVmax(reservoir.getVmin() + area * (maxlevel - minlevel));

        reservoir.setVcurve(c);
        reservoir.setMixModel(Reservoir.MixType.MIX1);
        reservoir.setV1max(1.0);
