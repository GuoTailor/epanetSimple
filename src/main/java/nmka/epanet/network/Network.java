...
public class Network {
    ...
    public enum SectType {
        ...
        SectType(int id, String text) {
            this.id = id;
            this.parseStr = text;
            this.reportStr = "";
        }

        SectType(int id, String text, String repStr) {
            this.id = id;
            this.parseStr = text;
    ...
    private transient Map<String, Reservoir> reservoirs;
    private final List<String> titleText;
    /**
    ...
    public void addReservoir(String id, Reservoir reservoir) {
        reservoir.setId(id);
        if (reservoirs == null)
            reservoirs = new LinkedHashMap<String, Reservoir>();
        reservoirs.put(id, reservoir);
        nodes.put(id, reservoir);
    }
    ...
    public Collection<Reservoir> getReservoirs() {
        if (reservoirs == null) {
            Map<String, Reservoir> tempReservoir = new LinkedHashMap<String, Reservoir>();
            for (Node n : nodes.values()) {
                if (n instanceof Reservoir)
                    tempReservoir.put(n.getId(), (Reservoir) n);
            }
            if (tempReservoir.size() == 0)
                return new ArrayList<Reservoir>();
            else
                reservoirs = tempReservoir;
        }
    ...
    public String toString() {
        ...
        if (reservoirs != null) res += "  Reservoirs : " + reservoirs.size() + "\n";
